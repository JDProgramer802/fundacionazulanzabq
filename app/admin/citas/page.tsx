...
import { Calendar, Clock, User, Phone, Mail, MessageSquare, Loader2, CheckCircle2, XCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {appointments.map((app) => (
          <div key={app.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex justify-between items-start">
                ...
                <span className={`text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full ${
                  ...
                  app.status === 'completada' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                }`}>
                  {app.status}
                </span>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 mt-1">
                   <Clock size={14} /> {new Date(app.datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
